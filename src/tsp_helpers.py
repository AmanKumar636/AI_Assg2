...
def total_distance(tour, distance_matrix):
    """Calculate total distance for a TSP tour (returns to start)."""
    dist = 0
    n = len(tour)
    for i in range(n):
...
def generate_distance_matrix(cities):
    """Generate matrix of Euclidean distances between cities."""
    n = len(cities)
...
def generate_initial_solution(n):
    """Return a random permutation of city indices as an initial TSP tour."""
    tour = list(range(n))
    random.shuffle(tour)
    return tour

def generate_neighbors(tour):
    """Generate neighbor solutions by swapping two cities in the tour."""
    neighbors = []
    n = len(tour)
    for i in range(n):
